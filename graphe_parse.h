...
/**
 * @file graphe_parse.h
 * @brief Module `graphe_parse` qui permet de praser un fichier texte décrivant
 * un graphe.
 *
 ...
 * @endcode
 *
 * Avec `identifiant` toute chaîne sans ':' et sans '->', et `nombre` un nombre
 * flottant valide (incluant en notation scientifique).
 *
 * Le parseur ignore les espaces simples (" ") en dehors des identifiants mais
 * les retours à la ligne sont importants.
 *
 * Une description de noeud se compose donc d'une liste de noeuds, chacun étant
 * un nom associé à un couple (sa position).
 *
 * Une description de lien est de la forme `- A -> B` ou `- A -- B` avec A et B
 * des noeuds préalablement définis. Dans le cas `->` un arc de A vers B est
 * créé ; dans le cas `--` l'arc A vers B et B vers A sont tous deux créés (pour
 * faciliter l'écriture).
 *
 * Un graphe se compose de la section des noeuds puis de la section des liens
 * (dans cet ordre). Les lignes vides sont ignorées.
 *
 * Voici un example :
 * @code
 * Noeuds:
 ...
 *
 * Le graphe alloué a pour information sur les noeuds (dans le "noeud données")
 * le nom du noeud tel que fourni dans le fichier.
 *
