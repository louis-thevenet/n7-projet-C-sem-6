...
/**
 * Implémenation du type pour la fenêtre. Contient principalement une
 * `SDL_Window` et son `SDL_Renderer`, une texture cible représentant le contenu
 * de la fenêtre, la fonction `repaint` donnée par l'utilisateur et diverses
 * choses pour la concurrence.
 */
struct window_t {
  ...
  /** Texture cible du contenu de la fenêtre, sur laquelle le rendu est
   * effectué. */
  SDL_Texture *target;
  /** Couleur de fond */
  ...
  /** Callback pour rendre les éléments dans la fenêtre, appelé par le render
   * thread. */
  repaint_fn repaint;
  /** Données utilisateur passées au moment de créer la fenêtre et transmises au
   * callback. */
  void *userdata;

  /** Le render thread. */
  SDL_Thread *render_thread;
  /** Verrour sur cet enregistrement (partagé entre le thread principal et le
   * render thread */
  SDL_mutex *mutex;
  /** Sémaphore qui indique la fenêtre est "prête", autrement dit qu'elle a
   * terminé l'initialisation et la tâche de rafraîchissement (signalisé par la
   * fenêtre).
   */
  SDL_sem *ready;
  /** Sémaphore pour "réveiller" la fenêtre et initier le rafraîhcissement (en
   * attente par le render thread).
   */
  SDL_sem *wake_up;

  /** Commande pour stopper la fenêtre (si stop == true au prochian réveil le
   * render-thread se termine). */
  bool stop;
};

static void color_restore(struct window_t *window) {
  SDL_SetRenderDrawColor(window->renderer, window->background.r,
                         window->background.g, window->background.b,
                         window->background.a);
}

/**
 * Fonction du render thread qui réalise la gestion du rafraîchissement de la
 * fenêtre.
 *
 * Pour des raisons de cohérence du multi-threading, c'est en fait cette
 * fonction qui se charge de configurer la fenêtre (créer le `SDL_Window` et
 * associés). De cette façon, on est sûr que la texture cible, le renderer et
 * les textures crées pour le rafraîchissement sont bien dans le même thread.
 *
 * @param userdata donnée utilisateurs passées au moment de la création du
 * thread, qui contiennent ici une référence vers la window à laquelle le thread
 * est attachée.
 */
static int do_render(void *userdata) {
  ...
  window->stop = false;
  SDL_CreateWindowAndRenderer(window->w, window->h,
                              0 /* SDL_WINDOW_RESIZEABLE */, &window->window,
                              &window->renderer);
  if (window->name != NULL) {
    SDL_SetWindowTitle(window->window, window->name);
  }
  // La texture cible (sur laquelle toutes les autres seront rendues)
  window->target = SDL_CreateTexture(
      window->renderer, 0, SDL_TEXTUREACCESS_TARGET, window->w, window->h);
  ...
  while (cue) {
    ...
    SDL_UnlockMutex(window->mutex); // Fin de zone critique (on fait un lock en
                                    // deux temps pour être plus efficace)
...
struct window_t *create_window(int w, int h, const char *name,
  ...
  // On ne crée pas ces éléments tout de suite car on veut les créer dans le
  // render thread !
  win->window = NULL;
  win->renderer = NULL;
  win->target = NULL;
  ...
  win->loaded_fonts =
      (TTF_Font **)malloc(win->fontreg_size * sizeof(TTF_Font *));
...
void start_window(struct window_t *window) {
  ...
  SDL_UnlockMutex(window->mutex);
  SDL_SemWait(window->ready); // Le sémaphore est signalisé lorsque le thread
                              // rentre dans la boucle (ce qu'il fait que si il
                              // a fini son initialisation)
}
...
void refresh_window(struct window_t *window) { SDL_SemPost(window->wake_up); }

/**
 * Fonction auxiliaire qui vérifie que le thread courant est bien le render
 * thread de la window.
 *
 * Cette fonction ne fait quelque chose que si `STATIC_THREADING` est activé
 * (sinon elle ne fait rien).
 *
 * Si le thread n'est pas bon, cela compte comme une erreur critique et le
 * programme est quitté brutalement avec le code d'erreur -1.
 *
...
font_id_t load_font(struct window_t *window, const char *fontname, int ptsize) {
  int fontid = 0;
  while (fontid < window->fontreg_size &&
         window->loaded_fonts[fontid] != NULL) {
    fontid++;
  }

  if (fontid >= window->fontreg_size) {
    TTF_Font **tmp =
        (TTF_Font **)realloc(window->loaded_fonts, window->fontreg_size * 2);
    if (tmp == NULL) {
      return -1;
    }
...
void render_line(struct window_t *window, int x1, int y1, int x2, int y2,
                 SDL_Color color) {
  test_thread(window);
  aalineRGBA(window->renderer, x1, y1, x2, y2, color.r, color.g, color.b,
             color.a);
}
...
void render_line_thick(struct window_t *window, int x1, int y1, int x2, int y2,
  ...
  test_thread(window);
  thickLineRGBA(window->renderer, x1, y1, x2, y2, thickness, color.r, color.g,
                color.b, color.a);
}
 ...
 */
static void arrow_sides(int ex, int ey, float dx, float dy, float alpha,
                        float size, int *sx1, int *sy1, int *sx2, int *sy2) {
  double c = cos(alpha);
  double s = sin(alpha);
...
void render_arrow(struct window_t *window, int x1, int y1, int x2, int y2,
  ...
  arrow_sides(x2, y2, (float)(x1 - x2) / length, (float)(y1 - y2) / length,
              alpha, size, &lx, &ly, &rx, &ry);

  thickLineRGBA(window->renderer, x1, y1, x2, y2, thickness, color.r, color.g,
                color.b, color.a);
  thickLineRGBA(window->renderer, x2, y2, lx, ly, thickness, color.r, color.g,
                color.b, color.a);
  thickLineRGBA(window->renderer, x2, y2, rx, ry, thickness, color.r, color.g,
                color.b, color.a);
}
...
void render_arrow_curved(struct window_t *window, int x1, int y1, int x2,
  ...
  // Arrow head
  int lx, ly, rx, ry;
  arrow_sides(xs[3], ys[3], (float)(xs[2] - xs[3]) / n23,
              (float)(ys[2] - ys[3]) / n23, alpha, size, &lx, &ly, &rx, &ry);

  bezierRGBA(window->renderer, xs, ys, 4, 2, color.r, color.g, color.b,
             color.a);
  aalineRGBA(window->renderer, xs[3], ys[3], lx, ly, color.r, color.g, color.b,
             color.a);
  aalineRGBA(window->renderer, xs[3], ys[3], rx, ry, color.r, color.g, color.b,
             color.a);
}
...
void render_text(struct window_t *window, const char *text, font_id_t fontid,
  ...
  SDL_Surface *textsurf =
      TTF_RenderUTF8_Solid(window->loaded_fonts[fontid], text, color);
  SDL_Rect textarea;
  ...
  SDL_SetRenderDrawColor(window->renderer, background.r, background.g,
                         background.b, background.a);
  SDL_RenderFillRect(window->renderer, &textarea);

  SDL_Texture *texture =
      SDL_CreateTextureFromSurface(window->renderer, textsurf);
