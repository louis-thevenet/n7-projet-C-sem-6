 ...
 * de préférence.
 *
 * Le principe est de les tester une par une jusqu'à ce qu'une d'entre elles
 * marche...
 *
 * Si ça n'arrive pas ça risque de mal se passer (le code n'est pas très robuste
 * de ce côté là :| )
 */
static const char *PossibleFonts[] = {
    // Toutes ces font sont monospace/sans serif
    "TlwgMono.ttf",
    ...
    NULL // dernière case = NULL pour indiquer la fin du tableau lorsqu'on le
         // parcours
};
 ...
 * @return police nouvellement chargée, ou NULL si aucune des polices de @ref
 * PossibleFonts n'a pu être chargée
 */
...
struct graphrep_t *create_graphrep(struct window_t *window,
                                   ...
                                   viewport_t viewport) {
  struct graphrep_t *result =
      (struct graphrep_t *)malloc(sizeof(struct graphrep_t));
  result->graph = graph;
  ...
  result->path = NULL;
  result->node_styles = (struct node_style_t *)calloc(
      nombre_noeuds(graph), sizeof(struct node_style_t));
 ...
 * @param udata données utilisateurs transmises par `pour_chaque_noeud`,
 * normalement un `minmax_t`.
 */
...
void highlight_node(struct graphrep_t *graphrep, noeud_id_t node,
  ...
  while (i < nnoeuds && graphrep->node_styles[i].node != NO_ID &&
...
static struct node_style_t get_node_style(const struct graphrep_t *graphrep,
                                          noeud_id_t id) {
  size_t nnoeuds = nombre_noeuds(graphrep->graph);
  size_t i = 0;
  while (i < nnoeuds && graphrep->node_styles[i].node != NO_ID &&
         graphrep->node_styles[i].node != id) {
    i++;
  }
...
/**
 * Dessiner un noeud. Cette fonction est appelée par @ref pour_chaque_noeud
 * depuis la fonction appelée par la fenêtre (donc dans le thread de rendu).
 *
...
static void draw_node(struct noeud_info_t info, void *userdata) {
  ...
  float m2c_dist = NORM((float)(center.x - data->graphrep->mouse.x),
  ...
  if (data->graphrep->path != NULL &&
      contient_noeud_liste(data->graphrep->path, info.id)) {
    color = RED;
  } else {
    color = BLACK;
  }
  ...
  render_text(data->window, (char *)info.noeud_data, data->graphrep->node_font,
              center.x, center.y, HALIGN_CENTER | VALIGN_MIDDLE, color,
              nodestyle.color);
...
/**
 * Dessiner un arc. Cette fonction est appelée par @ref pour_chaque_arrete
 * depuis la fonction appelée par la fenêtre de rendu (donc dans le thread de
 * rendu).
 *
...
static void draw_edge(struct arrete_info_t info, void *userdata) {
  ...
  // render_line_thick(data->window, src.x, src.y, dst.x, dst.y, EDGE_WIDTH,
  // GRAY); const int skip = NODE_SIZE; const float length = NORM(dst.x - src.x,
  // dst.y - src.y);
  SDL_Color color;

  if (data->graphrep->path != NULL &&
      contient_arrete_liste(data->graphrep->path, info.id_source,
                            info.id_destination)) {
    color = RED;
  ...
  render_arrow_curved(data->window, src.x, src.y, dst.x, dst.y, 8.0, 0.4, 25.0,
                      NODE_SIZE + NODE_SKIP, NODE_SIZE + NODE_SKIP, color);
}
