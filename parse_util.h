 ...
 *
 * La fonction ignore naturellement les espaces (caractères validant
 * `est_espace`) et retourne l'indice après lequel le dernier token a été
 * trouvé, ou -1 si la ligne ne commence pas par _tous_ les tokens donnés.
 *
 * Si un token se termine par `\>` on considère qu'il est _borné_, c'est à dire
 * que le token s'arrête lorsqu'on rencontre un délimiteur. Par exemple :
 * `commencer("abc:", "abc", NULL)` renvoie -1, mais `commencer("abc:",
 * "abc\\>", NULL)` renvoie 3
 *
 * @param entree chaîne à vérifier
 * @return indice de la fin du dernier token ou -1 si la chaîne ne commence pas
 * par les token donnés
 */
int commence(const char *entree, ...);

/**
 * Parse la chaîne en incrémentant l'indice passé en paramètre jusqu'à ce que le
 * caractère à l'indice `*i` soit c. Le fonction retourne vrai ssi le caractère
 * c a été rencontré, et alors `str[*i] == c`.
 *
...
/**
 * Parse la chaîne en incrémentant l'indice passé en paramètre jusqu'à ce que
 * les deux caractères consécutifs soient c1 puis c2. Retourne vrai ssi les
 * caractères sont présents dans la chaîne.
 *
 * Dit autrement : si le résultat est vrai, alors `str[*i] == c1` et `str[*i +
 * 1] == c2`
 *
 * @param str chaîne à parcourir
 * @param i indice à incrémenter
