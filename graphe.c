...
#error                                                                         \
    "Le système cible ne semble pas supporter la norme IEEE 754, nécessaire pour la compilation de ce module."
...
struct graphe_noeud {
   ...
   * normalement on a pas besoin de plus (!), mais si le graphe n'est pas
   * fortement connexe, ça fait pas mal d'espace gaspillé... En échange, c'est
   * rapide et pratique.
   */
  noeud_id_t *voisins;
};
...
struct graphe_t {
  ...
  /** Tableau des pointeurs sur les noeuds du graphe (les noeuds sont alloués
   * sur le tas) */
 ...
 * La façon dont l'identifiant est créé est caché, mais il fait intervenir le
 * temps CPU en nanosecondes, donc la probabilité de collision à num égal est
 * très faible
 * @param num numéro du noeud dans le graphe (numéro de la case dans le champ
 * noeuds)
...
static noeud_id_t creer_id(size_t num) {
  ...
  noeud_id_t id = ((unsigned long)((spec.tv_nsec & 0xFFFFFFFF) << 32)) |
 ...
 */
static size_t get_num(noeud_id_t id) { return (size_t)(id & 0xFFFF); }

/**
 * Helper qui vérifie si le noeud donné est valide (i.e. il existe dans le
 * graphe et il ne correspond pas à @ref NO_ID). Si le noeud n'est pas valide,
 * affiche un message en se servant des informations données. Si le noeud est
 * valide, met le numéro associé dans la variable getnum.
 ...
 */
static bool check_id(const struct graphe_t *graphe, const char *loc,
                     noeud_id_t id, size_t *getnum) {
  ...
  if (num > graphe->nombre_noeuds) {
    fprintf(stderr,
            "Accès à un noeud hors du graphe (id=%016lx, fonction %s)\n", id,
            loc);
    return false;
  }
...
static struct graphe_noeud *allouer_noeud(size_t num) {
  struct graphe_noeud *noeud =
      (struct graphe_noeud *)malloc(sizeof(struct graphe_noeud));
 ...
 * @return graphe nouvellement alloué, avec de la place pour nnoeuds noeuds et
 * initialement aucun noeud dedans.
 */
static struct graphe_t *allouer_graphe(size_t nnoeuds) {
  ...
  graphe->noeuds =
      (struct graphe_noeud **)malloc(nnoeuds * sizeof(struct graphe_noeud));
  return graphe;
}
...
noeud_id_t creer_noeud(struct graphe_t *graphe, point_t position,
  ...
  graphe->noeuds[num]->voisins =
      (noeud_id_t *)malloc(graphe->nombre_noeuds_max * sizeof(noeud_id_t));
...
bool ajouter_arrete(struct graphe_t *graphe, noeud_id_t source,
  ...
  if (!check_id(graphe, "creer_arrete", source, &num_src) ||
      !check_id(graphe, "creer_arrete", destination, &num_dst)) {
    return false;
  }
  ...
  size_t idv = 0;
  while (idv < noeud_src->nombre_voisins &&
...
bool est_voisin(const struct graphe_t *graphe, noeud_id_t source,
                noeud_id_t destination) {
  size_t num_src;

  if (!check_id(graphe, "est_voisin", source, &num_src) ||
      !check_id(graphe, "est_voisin", destination, NULL)) {
    return false;
  }
...
float noeud_distance(const struct graphe_t *graphe, noeud_id_t noeuda,
                     noeud_id_t noeudb) {
  size_t numa;
  if (!check_id(graphe, "distance", noeuda, &numa) ||
      !check_id(graphe, "distance", noeudb, NULL)) {
    return NAN;
  }
...
void exporter_dot(const struct graphe_t *graphe, FILE *sortie,
  ...
  for (size_t i = 0; i < graphe->nombre_noeuds; i++) {
    ...
    if (afficher_donnees == NULL) {
      fprintf(sortie,
              "    N%lu[label=\"\\#%lu (%f,%f)\" tooltip=\"<%016lx>\"];\n", num,
              num, noeud->position.x, noeud->position.y, noeud->id);
    } else {
      fprintf(sortie, "    N%lu[label=\"\\#%lu (%f,%f)\" tooltip=\"<%016lx> (",
              num, num, noeud->position.x, noeud->position.y, noeud->id);
  ...
  for (size_t i = 0; i < graphe->nombre_noeuds; i++) {
    ...
    for (size_t j = 0; j < noeud->nombre_voisins; j++) {
      ...
      fprintf(sortie, "    N%lu -> N%lu [label=\"%f\"];\n", numi, numj,
              distance(noeud->position, graphe->noeuds[numj]->position));
    }
  }
