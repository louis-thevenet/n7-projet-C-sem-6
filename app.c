 ...
 *
 * Dans les grandes lignes, ce type ne fait que maintenir ensemble les
 * composants et la config de l'app.
 */
struct app_t {
 ...
 * @param userdata donnée utilisateur donnée à la création du thread, qui
 * contient l'app
 */
...
void refresh_app(struct app_t *app) { refresh_window(app->window); }
 ...
 *
 * @param userdata données user transmise au moment de la création du thread
 * (les task data ici)
...
void exec_task_app(struct app_t *app, task_fun_t task, post_task_fun_t post,
  ...
  struct __task_data_t *data =
      (struct __task_data_t *)malloc(sizeof(struct __task_data_t));
  data->app = app;
  data->task = task;
  data->post = post;
