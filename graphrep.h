...
/**
 * @file graphrep.h
 * @brief Module graphrep qui définit un composant de représentation de graphe,
 * à embarquer dans une @ref window_t.
 *
 * Les noeuds sont représentés avec des cercles et les arcs par des flèches
 * incurvées.
 *
 * Le composnat permet de mettre en valeur un chemin, et réagit lorsqu'on
 * survole les noeuds.
 *
 * @author G. Dupont
 * @version 1.0
...
#define NODE_SKIP 0
/** Paramètre de représentation : largeur du trait pour les arcs (non
 * fonctionnel) */
#define EDGE_WIDTH 2
...
/**
 * Crée un composant de représentation de graphe pour la fenêtre donnée et avec
 * le graphe donné. Le composant représente le graphe sur la zone réelle donnée
 * (area) et avec le viewport donné, qui convertit les coordonnées des noeuds en
 * coordonnées d'écran. Cela permet de représenter n'importe quel système de
 * coordonnée du graphe.
 *
 ...
 * @param viewport transformation entre écran et système de coordonnées du
 * graphe
 * @return composant de représentation, alloué sur le tas
 */
struct graphrep_t *create_graphrep(struct window_t *window,
                                   struct graphe_t *graph, SDL_Rect area,
                                   viewport_t viewport);

/**
 * Crée un composant de représentation de graphe pour la fenêtre donnée et avec
 * le graphe donné, pour la zone d'écran donné. Le viewport est calculé
 * automatiquement à partir de l'aire occupée par le graphe et de la zone
 * donnée.
 *
 * Pré-conditions : window non NULL, graph non NULL
 *
 ...
 * @return composant de représentation, alloué sur le tas
 */
struct graphrep_t *create_graphrep_with_vp(struct window_t *window,
                                           struct graphe_t *graph,
                                           SDL_Rect area);

/**
 * Détruit le composant de représentation de graphe (libère toute la mémoire
 * allouée). Ceci ne libère pas le graphe.
 *
 ...
 *
 * Cette fonction affiche dans une couleur différente les noeuds du chemin, et
 * les arrêtes (ces dernières étant les couples (précédent(noeud), noeud)
 * stockés dans le chemin).
 *
 ...
 */
void highlight_node(struct graphrep_t *graphrep, noeud_id_t node,
                    SDL_Color color);

/**
 * Met en évidence l'arrête donnée (si elle existe). Il ne peut y avoir qu'une
 * seule arrête mise en évidence. Si l'arrête n'existe pas, aucune arrête n'est
 * mise en évidence, ce qui permet de retirer la mise en évidence au besoin.
 *
 * À noter que si une arrête est mise en évidence et appartient au chemin
 * actuellement mis en évidence, elle n'apparaît pas (priorité au chemin).
 *
 * Pré-conditions : graphrep != NULL
 *
 * @param graphrep composant à mettre à jour
 ...
 */
void highlight_edge(struct graphrep_t *graphrep, noeud_id_t source,
                    noeud_id_t dest);

/**
 * Recalcul le rendu du graphe. Cette fonction est appelée par la fenêtre lors
 * du repaint, et doit donc être appelée dans le thread de rendu.
 *
 * @param window fenêtre sur laquelle est rendu le graphe
 * @param userdata données utilisateur transmise par le thread, un graphrep en
 * théorie
 */
