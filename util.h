...
/** Écrétage d'une valeur entre deux valeurs (si x est en dessous de down,
 * ramène x à down, si x est au dessus de up, ramène x à up, sinon, laisse x
 * inchangé).
 */
...
#ifdef _DEBUG // Les fonctionnalités suivantes sont activées seulement en mode
              // debug
#include <stdlib.h>
...
#define TLog(...)                                                              \
  {                                                                            \
    printf("[thread-%08lx] ", SDL_ThreadID());                                 \
    printf(__VA_ARGS__);                                                       \
    putchar('\n');                                                             \
  }
/** Écrit un message de débug préfixé par "main" (pour le thread principal), à
 * la printf */
#define MLog(...)                                                              \
  {                                                                            \
    printf("[main] ");                                                         \
...
#else
/** Écrit un message de débug avec le numéro du thread (ne fait rien dans cette
 * configuration) */
#define TLog(...)                                                              \
  {}
/** Écrit un message de débug préfixé par "main" (ne fait rien dans cette
 * configuration) */
#define MLog(...)                                                              \
  {}
#endif
