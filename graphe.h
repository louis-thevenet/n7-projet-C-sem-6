...
/**
 * @file graphe.h
 * @brief Module graphe, qui définit un graphe orienté, dans les noeuds duquel
 * sont stocké une position cartésienne et des données utilisateurs (un nom, par
 * exemple).
 *
 * Les graphes sont implantés avec des tableaux (pour des raisons de practicités
 * et parce que le projet est autrement dépourvu de toute structure chaînée).
 * Une conséquence de cela est que l'on doit donner le nombre (maximum) de
 * noeuds présents dans le graphe. Dans ce sens, les graphes sont plutôt des
 * objets statiques ("immutables" d'une certaine façon).
 *
 * Le type pour les graphes est complètement abstrait. L'utilisateur n'a pas
 * accès aux noeuds directement, mais à des _identifiants_ de noeud (@ref
 * noeud_id_t).
 *
 * @author G. Dupont
 * @version 1.0
...
/** Identifiant spécial correspondant à un identifiant invalide, un noeud non
 * trouvé, etc. */
#define NO_ID 0
...
/**
 * Crée un graphe avec le nombre (maximal) de noeuds donné. Le graphe est alloué
 * sur le tas.
 *
...
/**
 * Créer un noeud dans le graphe, avec la position et les données utilisateur
 * données. La fonction renvoie l'identifiant du noeud créé en cas de succès ou
 * @ref NO_ID si la création n'est pas possible (car plus de place typiquement),
 *
 ...
 */
noeud_id_t creer_noeud(struct graphe_t *graphe, point_t position,
                       void *donnees);

/**
 * Ajouter une arrête dans le graphe entre les noeuds donnés. La fonction
 * renvoie vrai si l'ajout a été réalisé et faux sinon (typiquement, les noeuds
 * n'existent pas ou il n'y a plus de place pour les arrêtes).
 *
 * Pré-conditions : graphe non NULL
 *
 ...
 */
bool ajouter_arrete(struct graphe_t *graphe, noeud_id_t source,
                    noeud_id_t destination);

/**
 * Libérer le graphe alloué en mémoire par @ref creer_graphe. La fonction admet
 * une fonction de libération customisée pour libérer les données utilisateur
 * (si elles ont été allouées sur le tas, typiquement). Cette fonction peut être
 * NULL, auquel cas les données utilisateurs sont laissées telle quelles.
 *
 * Pré-conditions : graphe != NULL
 * Post-conditions : *graphe == NULL
 *
 * @param graphe pointeur sur le graphe à libérer
 * @param liberer_donnees fonction à appeler sur les données utilisateur
 * stockées sur les noeuds ; cette fonction prend en paramètre les données à
 * libérer
 */
 ...
 *
 * Pré-conditions : graphe non NULL, noeuds non NULL, noeuds de taille
 * suffisante
 *
...
/**
 * Renvoie vrai ssi les noeuds donnés sont _voisins_ dans le graphe. B est un
 * voisin de A ssi il existe un arc (orienté) dont l'origine est A et la
 * destination est B.
 *
 ...
 * @param noeudb noeud destination (dont on veut savoir s'il est voisin de la
 * source)
 * @return vrai ssi noeudb voisin de noeuda
 */
bool est_voisin(const struct graphe_t *graphe, noeud_id_t noeuda,
                noeud_id_t noeudb);
 ...
 * tableau passé en paramètre, qui doit être de taille suffisante (au moins
 * égale à @ref nombre_voisins).
 *
 ...
 */
void noeuds_voisins(const struct graphe_t *graphe, noeud_id_t noeud,
                    noeud_id_t *voisins);

/**
 * Récupère les données utilisateurs associée au noeud, transmises au moment de
 * l'appel à
 * @ref creer_noeud. S'il n'y a pas de donnée pour le noeud ou si le noeud est
...
/**
 * Calcule la distance directe entre deux noeuds, c'est-à-dire la distance entre
 * leurs positions _s'ils sont voisins_. La fonction retourne `INFINITY` si les
 * noeuds ne sont pas voisins, et `NAN` si au moins l'un des deux noeuds est
 * invalide.
 *
 * Pré-conditions : graphe non NULL
 *
 ...
 * @return distance entre les positions respectives de noeudb et noeuda si
 * noeudb est voisin de noeuda, `INFINITY` s'ils ne sont pas voisins, `NAN` si
 * noeuda ou noeudb est invalide
 */
float noeud_distance(const struct graphe_t *graphe, noeud_id_t noeuda,
                     noeud_id_t noeudb);

/**
 * Informations sur le noeud en train d'être visité, passées en paramètre de la
 * fonction donnée à l'itérateur @ref pour_chaque_noeud ou @ref trouver_noeud.
 */
struct noeud_info_t {
 ...
 * correspond aux informations du noeud en cours de visite, ainsi que des
 * données utilisateurs, transmises via @ref pour_chaque_noeud.
 */
 ...
 * @ref trouver_noeud. Les fonctions de ce type reçoivent les informations du
 * noeud en cours de visite, ainsi que des données utilisateur transmises par
 * @ref trouver_noeud.
 */
typedef bool (*noeud_predicat)(struct noeud_info_t, void *);

/**
 * Informations sur l'arrête en train d'être visitée, passées en paramètre de la
 * fonction donnée à l'itérateur @ref pour_chaque_arrete.
 */
 ...
 * Les fonctions de ce type reçoivent les informations sur l'arrête en cours de
 * visite, ainsi que des données utilisateurs transmises par @ref
 * pour_chaque_arrete
 */
typedef void (*arrete_iterateur)(struct arrete_info_t, void *);

/**
 * Trouver un noeud qui respecte le prédicat passé en paramètre. Cette fonction
 * renvoie l'identifiant du noeud trouvé, ou @ref NO_ID si aucun noeud n'a été
 * trouvé.
 *
 * Pré-conditions : graphe non NULL, predicat non NULL
 * Post-conditions : si resultat != NO_ID, alors resultat est un noeud de graphe
 * et predicat(resultat) (modulo effets de bord)
 *
 ...
 */
noeud_id_t trouver_noeud(const struct graphe_t *graphe, noeud_predicat predicat,
                         void *userdata);

/**
 * Parcours les noeuds du graphe (une fois) et applique la fonction donnée à
 * chacun d'entre eux.
 *
 ...
 */
void pour_chaque_noeud(struct graphe_t *graphe, noeud_iterateur iterateur,
                       void *userdata);

/**
 * Parcours les arrêtes du graphe (une fois) et applique la fonction donnée à
 ...
 * @param userdata données utilisateur à passer à l'itérateur
 */
void pour_chaque_arrete(struct graphe_t *graphe, arrete_iterateur iterateur,
                        void *userdata);

/**
 * Exporte un graphe au format DOT de Graphviz sur le fichier donné. Les données
 * utilisateur exportées vers le fichier à l'aide de la fonction
 * `afficher_donnees` fournie par l'utilisateur. Si cette dernière est nulle,
 ...
 * @param afficher_donnees fonction qui exporte les données utilisateur vers la
 * sortie ; elles apparaissent sous la forme de tooltip dans le graphe
 */
void exporter_dot(const struct graphe_t *graphe, FILE *sortie,
                  void (*afficher_donnees)(FILE *, void *));
