...
/**
 * check_chemin - Fonction type ASSERT custom pour vérifier qu'un chemin se
 * compose bien des noeuds donnés.
 *
 * Les noeuds donnés sont sous la forme d'une liste d'arguments (variadique) où
 * chaque argument est un identifiant de noeud, et terminant par NO_ID.
 *
 * La fonction vérifie que chaque noeud est bien dans la liste, et que les
 * successions de noeuds correspondent bien à la configuration (précédent(x), x)
 * dans la liste. Pour le premier noeud, on vérifie que le précédent est NO_ID.
 *
 ...
 *
 */
static void ASSERT_FUN(check_chemin, const struct graphe_t *graphe,
                       const liste_noeud_t *chemin, ...) {
  va_list attendus;
  ...
  while ((attendu = va_arg(attendus, noeud_id_t)) != NO_ID) {
    ...
    if (premiertour) {
      ...
      if (precedent_noeud_liste(chemin, attendu) != precedent) {
        FAILF("Le précédent du noeud <<%s>> est <<%s>>, il devrait être le "
              "noeud <<%s>>\n",
              (char *)noeud_donnees(graphe, attendu),
              (char *)noeud_donnees(graphe,
                                    precedent_noeud_liste(chemin, attendu)),
...
int main() {
  ...
  for (int i = 0; i < lignes; i++) {
    for (int j = 0; j < colonnes; j++) {
      if (i < lignes - 1) {
        ajouter_arrete(graphe, noeuds[i * colonnes + j],
