...
static const SDL_Color LIGHT_BLUE = {
    .r = 0xC0, .g = 0xC0, .b = 0xFF, .a = 0xFF};
static const SDL_Color LIGHT_GREEN = {
    .r = 0xC0, .g = 0xFF, .b = 0xC0, .a = 0xFF};
...
static void highlight_exploring(noeud_id_t src, noeud_id_t dst,
                                void *userdata) {
  struct app_t *app = (struct app_t *)userdata;
...
/**
 * Réalise l'algorithme de dijkstra (à appeler avec le lanceur de tâche de
 * l'app).
...
static int perform_dijkstra(struct app_t *app, void *userdata) {
  ...
  begin = clock();
  conf->resultat =
      dijkstra(conf->graphe, conf->source, conf->destination, &(conf->chemin)
#ifdef AVEC_ANIMATION
                                                                  ,
               callback, app
#endif
...
/***
 * Réalise le post-traitement de l'algorithme (donc principalement montrer le
 * chemin et afficher distance minimale et temps de calcul).
 * @param app application dans laquelle rendre le chemin
 * @param userdata donnée passées à la création de tâche (un @ref
 * dijkstra_conf_t normalement)
 * @return 0 si pas de problème
 */
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    fputs("Erreur : le programme attend un ficher contenant la description "
          "d'un graphe, un noeud source et un neoud destination\n",
          stderr);
    exit(-1);
  }
  ...
  if (graphe == NULL) {
    fputs("Le programme a été interrompu pour cause de problèmes dans le "
  ...
  if (source == NO_ID || destination == NO_ID) {
    if (source == NO_ID) {
      fprintf(stderr,
              "Le noeud de source demandé (%s) n'existe pas dans le graphe.\n",
              argv[2]);
    }
    if (destination == NO_ID) {
      fprintf(
          stderr,
          "Le noeud de destination demandé (%s) n'existe pas dans le graphe.\n",
          argv[3]);
    }
    exit(-1);
  }
        ...
        (event.type == SDL_KEYDOWN &&
         (event.key.keysym.sym == SDLK_q ||
          event.key.keysym.sym ==
              SDLK_ESCAPE))) { // croix/boutin escape/boutin Q => quitter
      quit = true;
    } else if (event.type == SDL_MOUSEMOTION) { // la souris bouge => mise à
                                                // jour de la croix/curseur
