 ...
 *
 * Les variables sont ré-initialisées au début de `parser_graphe`, si bien
 * qu'elle sont reliées à cette méthode spécifiquement (il ne s'agit pas d'un
 * état persistant du module, à la fin de la fonction les variables ne servent
 * plus à rien).
 *
 * @author G. Dupont
 * @version 1.0
...
/**
 * Type interne qui représente le mode du parseur : par encore entré dans une
 * section
 * (`NO_SECTION`), dans la section `Noeuds` (`NODE_SECTION`) ou dans la section
 * `Liens` (`EDGE_SECTION`).
 */
enum parser_mode { NO_SECTION, NODE_SECTION, EDGE_SECTION };
 ...
 * warning).
 *
 * Le noeud créé par cette fonction est celui à l'indice dont la valeur est
 * celle de `num_noeuds` avant la fonction (puisque cette valeur augmente
 * après).
 *
...
static bool parser_noeud(const char *ligne, size_t debut) {
  ...
  if (num_noeuds >= num_blocks_noeuds * TAILLE_BLOCK) {
    num_blocks_noeuds++;
    struct parser_noeud *tmp = (struct parser_noeud *)realloc(
        noeuds_courants,
  ...
  // Trim
  size_t fin = id - 1;
  while (est_espace(ligne[fin]) && fin > debut) {
    fin--;
  }

  // Nom du noeud
  noeuds_courants[num_noeuds].nom =
      (char *)malloc((fin - debut + 2) * sizeof(char));
  memcpy(noeuds_courants[num_noeuds].nom, ligne + debut,
  ...
  nfin = id - 1;
  while (est_espace(ligne[nfin]) && nfin > ndebut) {
    nfin--;
  }
  ...
  if (end < ligne + nfin) {
    fprintf(stderr, "Ligne %d, colonnes %ld-%ld: nombre mal formé.\n",
            ligne_num, ndebut + 1, nfin + 1);
    fprintf(stderr, "   |%s", ligne);
    fputs("    ", stderr);
    for (size_t i = 0; i < ndebut; i++) {
      fputc(' ', stderr);
    }
    for (size_t i = ndebut; i < nfin; i++) {
      fputc('^', stderr);
    }
    fputc('\n', stderr);
    return false;
  ...
  while (ligne[id] != '\0') {
    ...
    if (!erreur_affichee && (ligne[id] != '\0' && !est_espace(ligne[id]))) {
      fprintf(stderr,
              "Ligne %d, colonne %ld: attention, caractère(s) en trop sur la "
              "ligne (ignoré).\n",
              ligne_num, id + 1);
...
/**
 * Initialise le graphe avec les noeuds parseurs qui ont viennent de la seciton
 * `Noeuds`.
 */
static void initialiser_graphe() {
  ...
  for (size_t i = 0; i < num_noeuds; i++) {
    noeuds_courants[i].nid = creer_noeud(graphe, noeuds_courants[i].pos,
                                         (void *)noeuds_courants[i].nom);
  }
...
static noeud_id_t trouver_noeud_nom(const char *nom, size_t n) {
  ...
  while (i < num_noeuds && resultat == NO_ID) {
    if (n == strlen(noeuds_courants[i].nom) &&
        strncmp(noeuds_courants[i].nom, nom, n) == 0) {
 ...
 * l'indice "debut".
 *
 * La source du noeud est l'identifiant avant le "->" (modulo espaces préfixes
 * et tiret préliminaire) et la destination est tout ce qu'il y a après.
 *
 * @param ligne ligne à parser
...
static bool parser_lien(const char *ligne, size_t debut) {
  ...
  ndebut = id;
  nfin = strlen(ligne) - 1;
  while (est_espace(ligne[nfin]) && nfin > debut) {
    nfin--;
  }
...
struct graphe_t *parser_graphe(const char *filename) {
  ...
  ligne_num = 0;
  noeuds_courants =
      (struct parser_noeud *)malloc(TAILLE_BLOCK * sizeof(struct parser_noeud));
  num_blocks_noeuds = 1;
  num_noeuds = 0;
  ...
  while (getline(&ligne, &taille, fichier) >= 0) {
    ...
    if (commence(ligne, "Noeuds\\>", ":", NULL) >= 0) {
      ...
      if (mode == NODE_SECTION) {
        ...
        fprintf(stderr,
                "Ligne %d: déclaration de noeud ou d'arrête en dehors d'une "
                "section.\n",
                ligne_num);
        ok = false;
      }
      ...
      if (lid < strlen(ligne)) {
        fprintf(stderr,
                "Ligne %d: ligne inattendue (ni un début de section, ni un "
                "élément dans une section).\n",
                ligne_num);
        ok = false;
      }
    }
